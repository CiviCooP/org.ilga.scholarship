...
class CRM_Scholarship_Utils {
  ...
  public static function findActivityBySid($sid,$evaluatorId){
    ...
    $sql = 'select entity_id
            from   civicrm_value_sessions_prop_9 cv
            ...
            where  submission_id_115 = %1 and ac.contact_id =%2';
    return CRM_Core_DAO::singleValueQuery($sql,[
      1 => [$sid,'Integer'],
  ...
  static public function sessionVerdictResults(){
    ...
    $sql = "select act.id,
       ...
       cv.verdict_111 verdictvalue,
       cv.submission_id_115 sid,
       ov3.name                  verdict
       from     civicrm_activity act
         ...
         join   civicrm_value_sessions_prop_9 cv on act.id = cv.entity_id
         join   civicrm_option_value     ov3 on (ov3.value = cv.verdict_111 and ov3.option_group_id= 125)
         join   civicrm_contact proposer     on (proposer.id=ac1.contact_id)
         join   civicrm_contact evaluator    on (evaluator.id=ac2.contact_id)
         where  act.activity_type_id = 69    and act.is_current_revision =1";
